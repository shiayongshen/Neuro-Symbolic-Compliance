...
若變數名稱屬於法條或規則的合規結果，例如 `*_ok`、`*_executed`、`*_submitted`、`*_compliant`、`*_met`、`*_fulfilled`、`*_required` 等，且案例文字沒有明確提到其狀態，則不要自行填值。

- 不要因為看到相關法條就預設這些變數為 `true` 或 `false`
- 不要把規範中的「應然狀態」直接當成案例事實
- 這類變數只有在案例敘述明確指出已完成、未完成、已執行、未執行時才可加入 facts

如果案例只提供基礎事實，請只填基礎事實，讓規則推導出的狀態由 constraints 與 Z3 自行決定。

### 6. 非法案例的填值目標

如果輸入案例屬於違規、違法或不合規案例，facts 的目標不是把所有合規旗標補滿，而是提供足夠的基礎事實，讓至少一個違法條件在 Z3 中可被推導成立。

- 優先填入案例明示的違規事實、缺失事實、未完成事實、低於門檻的數值事實
- 不要把沒有明說的合規旗標一律設成 true
- 也不要為了讓模型看起來完整而把所有 `*_ok`、`*_executed`、`*_submitted` 旗標都補齊
- 如果案例敘述本身已表明某些義務未履行，就應如實設定相應的基礎變數，讓 constraints 反推出違法結果
